...
module.exports = function (app, saveSecurityConfig, getSecurityConfig) {
  ...
  if (app.securityStrategy) {
    ...
    app.post('/security/config', (req, res, next) => {
      if (
        app.securityStrategy.allowConfigure &&
        app.securityStrategy.allowConfigure(req)
      ) {
    ...
    app.post('/security/addUser', (req, res, next) => {
      if (
        app.securityStrategy.allowConfigure &&
        app.securityStrategy.allowConfigure(req)
      ) {
    ...
    app.post('/security/setPassword', (req, res, next) => {
      if (
        ...
        var config = getSecurityConfig(app)
        app.securityStrategy.setPassword(config, req.body, (err, config) => {
          if (err) {
            console.log(err)
            res.status(500)
            res.send(err)
    ...
    app.post('/security/deleteUser', (req, res, next) => {
      if (
        app.securityStrategy.allowConfigure &&
        app.securityStrategy.allowConfigure(req)
      ) {
        var config = getSecurityConfig(app)
        app.securityStrategy.deleteUser(config, req.body, (err, config) => {
          if (err) {
            console.log(err)
            res.status(500)
            res.send(err)
