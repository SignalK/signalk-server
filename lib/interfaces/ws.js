...
module.exports = function (app) {
  ...
  api.handlePut = function (context, path, value) {
    var spark = pathSources[path]
    if (spark) {
      spark.write({
        context: context,
    ...
    primus.on('connection', function (spark) {
        ...
        if (msg.updates) {
          ...
          msg.updates.forEach(update => {
            update.values.forEach(valuePath => {
              var currentSource = pathSources[valuePath.path]
              if (_.isUndefined(currentSource) || currentSource != spark) {
                debug(
                  'registered source for path %s = %s',
                  valuePath.path,
                  spark.id
                )
                pathSources[valuePath.path] = spark
              }
            })
          })
        }
      ...
      spark.on('end', function () {
        ...
        _.keys(pathSources).forEach(path => {
          if (pathSources[path] == spark) {
            debug('removing source for %s', path)
            delete pathSources[path]
          }
        })
      })
