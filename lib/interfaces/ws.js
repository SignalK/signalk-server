...
function handleRealtimeConnection (app, spark, onChange) {
  ...
  if (spark.query.rawInput === 'all') {
    startRawInput(app, spark)
  }
}
...
function startRawInput (app, spark) {
  const onRawInput = wrapWithverifyWS(app.securityStrategy, spark, input => {
    spark.write(input + '\n')
  })
  app.on('rawInput', onRawInput)
  spark.onDisconnects.push(() => {
    app.removeListener('rawInput', onRawInput)
  })
}
