...
module.exports = function(app) {
        ...
        findPluginsAndWebapps()
          .then(([plugins, webapps]) => {
            getLatestServerVersion().then(serverVersion => {
              var result = getAllModuleInfo(plugins, webapps, serverVersion)
  ...
  function getAllModuleInfo(plugins, webapps, serverVersion) {
    ...
    if (
      ...
      compareVersions(serverVersion, app.config.version) > 0
    ) {
  ...
  function getAuthor(package) {
    debug(package.name + ' author: ' + package.author)
    return (
      (package.author && (package.author.name || package.author.email)) +
      '' +
      (package.contributors || [])
        .map(contributor => contributor.name || contributor.email)
        .join(',') +
      '' +
      (package.name.startsWith('@signalk/') ? ' (Signal K team)' : '')
    )
  }
