...
module.exports = function(app) {
    ...
    start: function() {
      ...
      app.get("/appstore/install/:name/:version", (req, res, next) => {
        ...
        if (availablePlugins.indexOf(name) == -1) {
          ...
          if (pluginInstalling) {
            pluginInstallQueue[name] = { name: name, version: version };
          } else {
      ...
      app.get("/appstore/", (req, res, next) => {
        ...
        agent(
          "GET",
          "http://registry.npmjs.org/-/v1/search?text=signalk-node-server-plugin"
        )
          .end()
          .then(function(response) {
            ...
            agent(
              "GET",
              "http://registry.npmjs.org/-/v1/search?text=signalk-webapp"
            )
              .end()
              .then(function(response) {
  ...
  function makePluginTable(plugins, existing) {
    ...
    result += plugins["objects"].reduce(function(result, pluginInfo) {
      ...
      } else if (pluginInstallQueue[name]) {
  ...
  function installPlugin(name, version) {
    ...
    pluginsInstalled[name] = pluginInstalling;

    const npm = spawn("npm", ["install", name]);

    npm.stdout.on("data", data => {
      pluginsInstalled[name].output.push(data);
    ...
    npm.on("close", code => {
      debug("close: " + name);
      pluginsInstalled[name]["code"] = code;
      pluginInstalling = undefined;
      console.log(`child process exited with code ${code}`);

      if (Object.keys(pluginInstallQueue).length) {
        var next_name = Object.keys(pluginInstallQueue)[0];
        var info = pluginInstallQueue[next_name];
        delete pluginInstallQueue[next_name];
        installPlugin(next_name, info.version);
      }
    });
  }
};
