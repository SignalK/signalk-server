...
module.exports = function(app) {
    ...
    return modules.reduce(function(result, pluginInfo) {
      ...
      result += `
        <tr>
          ...
          <td>${getAuthorColumn(pluginInfo)}</td>
  ...
  function getAuthorColumn(pluginInfo) {
    return (
      (pluginInfo.package.author.name || pluginInfo.package.author.email) +
      "<i>" +
      (pluginInfo.package.contributors || [])
        .map(contributor => contributor.name || contributor.email)
        .join(",") +
      "</i>" +
      (pluginInfo.package.name.startsWith("@signalk/")
        ? " (Signal K team)"
        : "")
    );
  }
