      ...
      app.get("/plugins", function(req, res, next) {
        res.json(
          app.plugins.map(plugin => {
            ...
            var uiSchema = plugin.uiSchema;
            if (
              typeof uiSchema === "undefined" ||
              typeof uiSchema["configuration"] === "undefined" ||
              typeof uiSchema["configuration"]['ui:order'] === "undefined"
            ) {
              var order = [];
              Object.keys(plugin.schema.properties).forEach(name => {
                order.push(name);
              });
              if (typeof uiSchema == "undefined") {
                uiSchema = {};
              }
              if (typeof uiSchema["configuration"] === "undefined") {
                uiSchema["configuration"] = {};
              }

              uiSchema["configuration"]["ui:order"] = order;
            }
  ...
  fs
    ...
    .forEach(pluginName => {
      ...
      if (
        metadata &&
        metadata.keywords &&
        metadata.keywords.includes("signalk-node-server-plugin")
      ) {
