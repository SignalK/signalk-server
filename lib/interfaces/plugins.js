...
module.exports = function (app) {
  ...
  function handleMessageWrapper (app) {
    return (providerId, data) => {
      if (app.config.settings.enablePluginLogging) {
        var plugin = app.pluginsMap[providerId]
        if (!_.isUndefined(plugin)) {
          if (!plugin.logger) {
            plugin.logger = new (require('../../providers/log'))({
              app: app,
              discriminator: providerId
            })
          }
          plugin.logger.write(data)
        } else {
          debug(`unknown provider id: ${providerId}`)
        }
      }
      app.handleMessage(providerId, data)
    }
  }

  function registerPlugin (app, pluginName, metadata) {
    debug('Registering plugin ' + pluginName)
    const plugin = require(pluginName)(app, handleMessageWrapper(app))
