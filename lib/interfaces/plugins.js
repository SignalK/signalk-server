          ...
          _.sortBy(app.plugins, [
            ...
            const schema =
              typeof plugin.schema === 'function'
                ? plugin.schema()
                : plugin.schema
            const status = providerStatus[plugin.name]
  ...
  function doRegisterPlugin (app, pluginName, metadata, location) {
    const appCopy = _.assign({}, app, {
      ...
      setProviderStatus: msg => {
        app.setProviderStatus(pluginName, msg)
      },
      setProviderError: msg => {
        app.setProviderError(pluginName, msg)
      }
    })
    let plugin
    try {
      plugin = require(path.join(location, pluginName))(appCopy)
    } catch (e) {
      console.error(`${pluginName} failed to start: ${e.message}`)
      app.setProviderError(pluginName, `Failed to start: ${e.message}`)
      return
    }
    onStopHandlers[plugin.id] = []
