...
const getLogger = require('@signalk/streams/logging').getLogger
...
module.exports = function (app) {
  ...
  return {
    start: function () {
      ...
      app.get('/plugins/uiExtentions', function (req, res, next) {
        const items = app.plugins
          .filter(plugin => _.isUndefined(plugin.uiExtention) == false)
          .map(plugin => {
            let data = null
            try {
            ...
            if (
              (_.isUndefined(data.enabled) && !plugin.enabledByDefault) ||
              !data.enabled
            ) {
              return null
            }
            ...
            const menuItem =
              typeof plugin.uiExtention.menuItem === 'function'
                ? plugin.uiExtention.menuItem()
                : plugin.uiExtention.menuItem

            const mappings =
              typeof plugin.uiExtention.urlMappings === 'function'
                ? plugin.uiExtention.urlMappings()
                : plugin.uiExtention.urlMappings

            if (menuItem && mappings) {
              const fullMappings = {}
              _.keys(mappings).forEach(key => {
                fullMappings[key] = `/plugins/${plugin.id}/public/${
                  mappings[key]
                }`
              })
              return { menuItem: menuItem, mappings: fullMappings }
            }
          })
  ...
  function doRegisterPlugin (app, packageName, metadata, location) {
    ...
    if (plugin.uiExtention) {
      console.log(
        `mapping /plugins/${plugin.id}/public = ${location}${plugin.id}/public`
      )
      app.use(
        `/plugins/${plugin.id}/public`,
        express.static(`${location}${plugin.id}/public`)
      )
    }
  }
}
