...
module.exports = function (app) {
  ...
  function aclFilter (req, path, result) {
    console.log('aclFilter')
    return app.securityStrategy
      ? app.securityStrategy.filterReadREST(req, path, result)
      : result
  }
    ...
    start: function () {
      ...
      app.get(apiPathPrefix + '*', function (req, res, next) {
        ...
        if (
          ...
          return res.json(aclFilter(req, path, vessels))
        } else {
          for (var i in path) {
            var p = path[i]
        ...
        return res.json(aclFilter(req, path, last))
      })
