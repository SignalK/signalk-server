...
function Server(opts) {
  ...
  app.get('/installplugin/:id', (req, res, next) => {
    if ( app.availablePlugins.indexOf(req.params.id) == -1 )
    {
      res.send('Unknown Plugin ' + req.params.id)
    }
    else
    {
      const npm = spawn('npm', ['install', req.params.id])
      ...
      npm.on('close', (code) => {
        ...
        res.redirect('/installplugins')
      });
    }
  })

  app.get('/installplugins', (req, res, next) => {
    var html = fs.readFileSync(__dirname + '/installplugins.html', {encoding: 'utf8'})
    var insertionIndex = html.indexOf('</div>');
    var sliceToInsertion = html.slice(0, insertionIndex);
    var sliceToEnd = html.slice(insertionIndex);
    var result = sliceToInsertion;
...
function makePluginTable(app, plugins, existing)
{
  ...
  result += plugins["objects"].reduce(function(result, pluginInfo) {
    ...
    app.availablePlugins.push(pluginInfo.package.name)
    
    var plugin = existing(app, pluginInfo.package.name)
    
    result += '<tr>';
    result += '<td>'
    if ( !plugin ) {
      result += '<a href="/installplugin/' + pluginInfo.package.name + '"><button type="button" class="btn">Install ' + pluginInfo.package.version + '</button></a> '
    } else {
      result += plugin.version + ' Installed'
    }
    
    result += '</td><td>'
    if ( plugin && compareVersions(pluginInfo.package.version, plugin.version) > 0 )
    {
      result += '<a href="/installplugin/' + pluginInfo.package.name + '"><button type="button" class="btn">Update to ' + pluginInfo.package.version + '</button></a> '
    } else if ( plugin ) {
    ...
    result += '</td>'
    result += '<td><b>' + pluginInfo.package.name + '</b></td> ';
