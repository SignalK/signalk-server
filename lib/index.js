...
Server.prototype.start = function() {
  ...
  if(Object.keys(app.interfaces).length > 0 && (_.isUndefined(app.config.settings.mdns) || app.config.settings.mdns === true)) {
    availableInterfaces['mdns'](app); 
    debug("Starting interface 'mDNS'");
  } else {
    debug("Not loading 'mDNS' because of configuration");
  }
