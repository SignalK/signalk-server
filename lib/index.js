  ...
  app.get('/installplugins', (req, res, next) => {
    ...
    agent('GET', 'http://registry.npmjs.org/-/v1/search?text=signalk-node-server-plugin').end().then(function(response) {
      ...
      result += plugins["objects"].reduce(function(result, pluginInfo) {
        ...
        var plugin = getPlugin(app, pluginInfo.package.name)
        
        result += '<tr>';
        result += '<td>'
        if ( !plugin ) {
          result += '<a href="/installplugins/' + pluginInfo.package.name + '"><button type="button" class="btn">Install</button></a> '
        } else {
          result += 'v' + plugin.version + ' Installed'
        }
        result += '</td><td>'
        if ( plugin && compareVersions(pluginInfo.package.version, plugin.version) > 0 )
        {
          result += '<button type="button" class="btn">Update to v' + pluginInfo.package.version + '</button> '
        } else if ( plugin ) {
