...
function Server(opts) {
  ...
  app.handleMessage = function(providerId, data) {
    if (data && data.updates) {
      ...
      var info = app.deltaCountPerProvider[providerId]
      if (_.isUndefined(info)) {
        info = {
          count: 0,
          percent: 0
        }
        app.deltaCountPerProvider[providerId] = info
      }
...
Server.prototype.start = function() {
  ...
  app.deltaCount = 0
  app.deltaCountPerProvider = {}
  let lastIntervalDeltaCount = 0
  app.intervals.push(
    setInterval(() => {
      var wsClients = 0
      if (app.interfaces['ws']) {
        wsClients = app.interfaces['ws'].numClients()
      }
      calculateProviderData(app)
      app.emit('serverevent', {
        ...
        data: {
          ...
          wsClients: wsClients,
          deltaCountPerProvider: app.deltaCountPerProvider
        }
      })
      lastIntervalDeltaCount = app.deltaCount
      _.values(app.deltaCountPerProvider).forEach(info => {
        info.lastIntervalDeltaCount = info.count
      })
    }, 5 * 1000)
  )
...
function calculateProviderData(app) {
  _.values(app.deltaCountPerProvider).forEach(info => {
    info.rate = (info.count - info.lastIntervalDeltaCount) / 5
  })
  var total = _.reduce(
    app.deltaCountPerProvider,
    (result, value) => {
  ...
  _.values(app.deltaCountPerProvider).forEach(info => {
    info.percent = Math.round(info.rate / total * 100)
  })

  app.providers.forEach(provider => {
    if (_.isUndefined(app.deltaCountPerProvider[provider.id])) {
      app.deltaCountPerProvider[provider.id] = {
        count: 0,
        percent: 0,
        rate: 0
      }
