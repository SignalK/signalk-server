...
function Server(opts) {
  ...
  app.setProviderStatus = function(providerId, statusMessage, type) {
    ...
    if (type === 'error') {
      status.lastError = statusMessage
  ...
  app.getProviderStatus = function() {
    let providerStatus = JSON.parse(JSON.stringify(app.providerStatus))
      ...
      app.plugins.forEach(plugin => {
        if (typeof plugin.statusMessage === 'function') {
          ...
          if (message) {
            ...
            if (message.length > 0) {
              if (_.isUndefined(app.providerStatus[plugin.id])) {
                providerStatus[plugin.id] = {}
              }
              providerStatus[plugin.id].message = message
              providerStatus[plugin.id].type = 'normal'
            }
          }
        }
      })
