...
function Server(opts) {
  ...
  app.get('/showinstalloutput/:id/:version' , (req, res, next) => {
    ...
    result += '</pre>'
    result += '<a href="/installplugin/' + req.params.id + '/' + req.params.version + '"><button type="button" class="btn">Retry</button></a> '
    result += sliceToEnd;
    res.send(result)
  })

  app.get('/installplugin/:name/:version', (req, res, next) => {
  ...
  app.get('/installplugins', (req, res, next) => {
    ...
    agent('GET', 'http://registry.npmjs.org/-/v1/search?text=signalk-node-server-plugin').end().then(function(response) {
      ...
      if ( Object.keys(app.pluginsInstalled).length ) {
        result += '<p class="text-warning">Server restart is required to pickup new or updated plugins.</p>\n'
      }
  ...
  result += plugins["objects"].reduce(function(result, pluginInfo) {
    ...
    if ( app.pluginsInstalled[name] ) {
      if ( app.pluginInstalling && app.pluginInstalling.name == name )
      {
        ...
        result += '<a href="/showinstalloutput/' + name + '/' + app.pluginsInstalled[name].version + '"><button type="button" class="btn-danger">Error</button></a> '
      }
    } else if ( app.pluginInstallQueue[name] ) {
      result += '<p class="text-primary">Waiting...' + '</p>'
    } else if ( !plugin ) {
      result += '<a href="/installplugin/' + name + '/' + version + '"><button type="button" class="btn">Install ' + version + '</button></a> '
    } else {
    ...
    if ( !app.pluginsInstalled[name] ) {
      if ( plugin && compareVersions(version, plugin.version) > 0 ) {
        result += '<a href="/installplugin/' + name + '/' + version + '"><button type="button" class="btn">Update to ' + version + '</button></a> '
      } else if ( plugin ) {
