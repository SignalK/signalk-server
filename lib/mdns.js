...
module.exports = function mdnsResponder(app) {
  ...
  let txtRecord = {
    ...
    roles: "master, main",
    self: app.selfId,
    vessel_name: getConfigString(app.signalk.self, 'name'),
    //there's nothing in the signalk schema for these??
    //vessel_brand: getConfigString(config.settings.vessel, 'brand'),
    //vessel_type: getConfigString(config.settings.vessel, 'type'),
    vessel_mmsi: getConfigString(app.signalk.self, 'mmsi'),
    vessel_uuid: getConfigString(app.signalk.self, 'uuid')
  };
