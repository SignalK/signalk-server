...
module.exports = function (app) {
  ...
  // set app.selfType as well
  // check uuid and mmsi formats
  throw new Error('todo')

  if (
    typeof app.config.defaults === 'object' &&
    typeof app.config.defaults.vessels === 'object' &&
    typeof app.config.defaults.vessels.self === 'object' &&
    typeof app.config.defaults.vessels.self === 'object' &&
    app.config.defaults.vessels.self !== null &&
    (typeof app.config.defaults.vessels.self.uuid === 'string' ||
      typeof app.config.defaults.vessels.self.mmsi === 'string')
  ) {
  ...
  if (app.selfType) {
    ...
    if (app.selfType == 'mmsi') {
      app.config.defaults.vessels.self.uuid =
        'urn:mrn:signalk:mmsi:' + app.selfId
    }
  }
}
