...
(function() {
  ...
  // Wrapping in a try/catch because I'm too lazy to check the existence of all keys in config.settings. 
  try {
    this.app.selfType = 'uuid';
    this.app.selfId = this.app.config.settings.vessel.uuid;

    if(typeof this.app.selfId === "undefined" && this.app.config.settings.vessel.mmsi) {
      this.app.selfType = 'mmsi';
      this.app.selfId = this.app.config.settings.vessel.mmsi;
    }

    debug(this.app.selfType.toUpperCase() + ": " + this.app.selfId);
  } catch(e) {}
}).call(global);