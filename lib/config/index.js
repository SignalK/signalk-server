 ...
 */
 ...
 (function() {
  ...
  try {
    ...
    var settingsFile = path.normalize(__dirname + '/../../' + getSettingsFilename(app.argv));

    try {
      config.settings = require(settingsFile);
    } catch(e) {
      app.log("Error resolving settings file ('" + settingsFile + "'). Are you sure it's there?".red)
      config.settings = {};
    }
