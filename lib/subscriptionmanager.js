...
SubscriptionManager.prototype.subscribe = function(command, unsubscribes, errorCallback, callback) {
  ...
  if (Array.isArray(command.subscribe)) {
    handleSubscribeRows(command.subscribe, unsubscribes, this.streambundle.buses, contextFilter, callback)
    ...
    unsubscribes.push(this.streambundle.keys.onValue(key => {
        ...
        handleSubscribeRows(command.subscribe, unsubscribes, buses, contextFilter, callback)
    }))
  }
}

function handleSubscribeRows(rows, unsubscribes, buses, filter, callback) {
  rows.reduce((acc, subscribeRow) => {
    if (subscribeRow.path) {
      handleSubscribeRow(subscribeRow, unsubscribes, buses, filter, callback);
    }
    return acc
  }, unsubscribes)
}

function handleSubscribeRow(subscribeRow, unsubscribes, buses, filter, callback) {
