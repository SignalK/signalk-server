...
Provider.prototype.start = function() {
  this.log('Starting Provider', this.settings.type);
  
  var self = this;
  
  try {
    this.provider = require(path.normalize(__dirname + '/../../providers/' + this.settings.type));
    this.provider.init(self, function(data) { self.handleMessage.call(self, data) }, this.debug);
  } catch(err) {
    this.log("Error initializing provider:", err.stack);
  }
};
...
Provider.prototype.log = function() {
  ...
  var args = [ ('[' + date.getFullYear() + '-' + z(date.getMonth() + 1) + '-' + z(date.getDate()) + ' ' + z(date.getHours()) + ':' + z(date.getMinutes()) + ':' + z(date.getSeconds()) + '.' + date.getMilliseconds() + ']').white + ('[provider-' + this.settings.type + ']').yellow ];
