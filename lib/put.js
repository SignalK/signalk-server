  ...
  start: function (app) {
    ...
    app.put(apiPathPrefix + '*', function (req, res, next) {
      ...
      if (info.state === State.denied) {
        res.status(403)
      } else if (info.state === State.noSource) {
        res.status(400)
        res.send(
...
function registerActionHandler (context, path, source, data, callback) {
  ...
  actionHandlers[context][path][source] = { data: data, f: callback }
...
function actionResult (actionId, status) {
