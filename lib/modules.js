...
const _ = require('lodash/fp') // NOTE THIS IS THE FP VERSION.
...
const getModulePaths = _.flow(
  _.get('config'),
  // appPath is the app working directory.
  _.at(['configPath', 'appPath']),
  _.uniq, // Remove one if duplicate.
  _.map(_.partialRight(path.join, ['node_modules/'])) // Append path.
)
...
const priorityPrefix = (a, b) =>
  _.getOr('', 'module', a)
    .replace('@signalk', ' ')
    .localeCompare(_.getOr('', 'module', b).replace('@signalk', ' '))

// Searches for installed modules that contain `keyword`.
function modulesWithKeyword (app, keyword) {
  return _.flow(
    getModulePaths,
    // Get modules with keyword for each path.
    _.map(_.partialRight(findModulesInDir, [keyword])),
    _.flatten, // unnest since values are inside an array.
    _.sortBy(priorityPrefix)
  )(app)
}
