...
module.exports = function (app, config) {
    ...
    if (
      ...
      var notAllowed = delta.updates.find(update => {
        return update.values.find(valuePath => {
          return (
            strategy.checkACL(req.user.id, context, path, 'write') === false
          )
        })
      })
  ...
  strategy.filterReadDelta = (req, delta) => {
    if (delta.updates && getConfiguration().acls) {
      var context =
        delta.context === app.selfContext ? 'vessels.self' : delta.context
  ...
  strategy.filterReadREST = (req, path, result) => {
    if (getConfiguration().acls) {
      var context = path[0] + '.' + path[1]
      var rootKey = path.slice(2).join('.')
  ...
  strategy.checkACL = (id, context, path, operation) => {
    var configuration = getConfiguration()

    if (!configuration.acls) {
    ...
    if (acl) {
      var pathPerms = acl.paths.find(p => {
        return p.paths.find(p => {
