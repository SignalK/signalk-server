...
module.exports = function (app, config) {
  ...
  function setupApp () {
      ...
      login(name, password)
        .then(reply => {
          ...
          if (reply.statusCode === 200) {
            ...
            res.cookie('SK_COOKIE', reply.token, { httpOnly: true }) // Current spec requires a "SK_COOKIE" to be set..

            if (requestType === 'application/json') {
              res.json({
                token: reply.token,
                type: 'JWT' // current version only supports JWT
              })
            } else {
    ...
    app.use('/apps', httpAuthorize(false))
    app.use('/appstore', httpAuthorize(false))
    app.use('/plugins', httpAuthorize(false))
    app.use('/restart', httpAuthorize(false))
    app.use('/security', httpAuthorize(false))
    app.use('/vessel', httpAuthorize(false))
    app.use('/providers', httpAuthorize(false))
    app.use('/settings', httpAuthorize(false))
    app.use('/webapps', httpAuthorize(false))

    app.put('/logout', function (req, res) {
      res.clearCookie('JAUTHENTICATION')
      res.clearCookie('SK_COOKIE')
      res.send('Logout OK')
    })
    ...
    /* This method isn't used anywhere, nor exported. @sbender9 remove?
    ...
    app.use('/loginStatus', httpAuthorize(false, true))

    const noRedir = httpAuthorize(false)
    app.use('/signalk/v1/api/*', function (req, res, next) {
      noRedir(req, res, next)
    })
  ...
  function login (name, password) {
    return new Promise((resolve, reject) => {
      ...
      const user = configuration.users.find(user => user.username === name)
      if (!user) {
      ...
      bcrypt.compare(password, user.password, (err, matches) => {
        if (err) {
          ...
          const expiration = configuration.expiration || '1h'
  ...
  strategy.addAdminMiddleware = function (path) {
    app.use(path, httpAuthorize(false))
  ...
  strategy.deleteUser = (config, username, callback) => {
    assertConfigImmutability()
    for (let i = config.users.length - 1; i >= 0; i--) {
  ...
  strategy.updateDevice = (config, clientId, updates, callback) => {
    assertConfigImmutability()
    const device = config.devices.find(d => d.clientId === clientId)
  ...
  strategy.authorizeWS = function (req) {
    ...
    if (!token) {
      token =
        req.cookies && (req.cookies.SK_COOKIE || req.cookies.JAUTHENTICATION)
    }
  ...
  strategy.checkACL = (id, context, path, source, operation) => {
    ...
    const acl = configuration.acls.find(acl => {
      const pattern = acl.context.replace('.', '\\.').replace('*', '.*')
  ...
  function getPrincipal (payload) {
    let principal
    if (payload.id) {
      const user = options.users.find(user => user.username === payload.id)
      if (user) {
        principal = {
  ...
  function httpAuthorize (redirect, forLoginStatus) {
    // debug('httpAuthorize: ' + redirect)
    return function (req, res, next) {
      let token = req.cookies.SK_COOKIE || req.cookies.JAUTHENTICATION

      debug(`httpAuthorize: ${req.path} (forLogin: ${forLoginStatus})`)
  ...
  strategy.setAccessRequestStatus = (config, identifier, status, body, cb) => {
    ...
    if (status === 'approved') {
      if (request.clientRequest.accessRequest.clientId) {
        ...
        const token = jwt.sign(payload, config.secretKey, jwtOptions)
