...
module.exports = function(app, config) {
  ...
  let { users = [], immutableConfig = false } = config
  ...
  strategy.shouldAllowPut = function(req, context, source, path) {
    if (
      ...
      const context = context === app.selfContext ? 'vessels.self' : context
  ...
  strategy.authorizeWS = function(req) {
    ...
    //
    const user = configuration.users.find(user => user.username === payload.id)
    if (!user) {
      error = new Error('Invalid access token')
      debug(error.message)
      throw error
