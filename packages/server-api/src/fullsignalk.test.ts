...
describe('Source freshness tracking', function () {
  it('stamps lastSeen on N2K sources (in sourceMeta, not on tree)', function () {
    ...
    const meta = fullSignalK.sourceMeta['can0.44']!
    expect(meta).to.exist
    expect(meta.lastSeen).to.be.at.least(before)
    expect(meta.lastSeen).to.be.at.most(after)
    // The /sources tree must NOT carry lastSeen — it would break the
    // Signal K spec schema's banUnknownProperties check.
    const leaf = fullSignalK.retrieve().sources['can0']['44']
    expect(leaf).to.not.have.property('lastSeen')
  })

  it('stamps lastSeen on NMEA 0183 talker (in sourceMeta)', function () {
    const fullSignalK = new FullSignalK('urn:mrn:imo:mmsi:200000000')
    fullSignalK.addDelta({
      context: 'vessels.urn:mrn:imo:mmsi:200000000',
      updates: [
    ...
    expect(fullSignalK.sourceMeta['0183-1.II']!.lastSeen).to.be.a('number')
  })

  it('stamps lastSeen on $source-only sources (in sourceMeta)', function () {
    const fullSignalK = new FullSignalK('urn:mrn:imo:mmsi:200000000')
    fullSignalK.addDelta({
      context: 'vessels.urn:mrn:imo:mmsi:200000000',
      updates: [
    ...
    expect(fullSignalK.sourceMeta['1W.0316013faeff']!.lastSeen).to.be.a(
      'number'
    )
  })
        ...
        updates: [
          {
            ...
            timestamp,
            values: [{ path: 'environment.water.temperature', value: 15.2 }]
          }
        ]
      })
    send(130312, '0', '2024-01-01T00:00:00.000Z')
    send(130312, '1', '2024-01-01T00:00:01.000Z')
    send(130312, '0', '2024-01-01T00:00:02.000Z')
    send(127508, '2', '2024-01-01T00:00:03.000Z')

    const meta = fullSignalK.sourceMeta['can0.44']!
    expect(meta.pgnInstances).to.exist
    expect(meta.pgnInstances!['130312']).to.deep.equal([0, 1])
    expect(meta.pgnInstances!['127508']).to.deep.equal([2])
    // pgnInstances must NOT appear on the n2k tree leaf.
    const n2k = fullSignalK.retrieve().sources['can0']['44'].n2k
    expect(n2k).to.not.have.property('pgnInstances')
  })

  it('does not record pgnInstances when source has no instance field', function () {
    const fullSignalK = new FullSignalK('urn:mrn:imo:mmsi:200000000')
    fullSignalK.addDelta({
      context: 'vessels.urn:mrn:imo:mmsi:200000000',
      updates: [
    ...
    expect(fullSignalK.sourceMeta['can0.44']!.pgnInstances).to.equal(undefined)
  })
})
...
describe('Delta with source.instance', function () {
  ...
  it('does not split a dotted $source suffix into phantom keys', function () {
    const msg = {
      context: 'vessels.foo',
      updates: [
        {
          // Remote SK providerId contains dots in hostname / IP
          $source: 'ws-pi4.local:3000 (192.168.0.125).pi4.local:3000',
    ...
    const fullSignalK = new FullSignalK()
    fullSignalK.addDelta(msg)
    const sources = fullSignalK.retrieve().sources
    expect(sources).to.have.property('ws-pi4')
    expect(sources['ws-pi4']).to.have.property(
      'local:3000 (192.168.0.125).pi4.local:3000'
    )
    // Phantom chain from naive split should not exist.
