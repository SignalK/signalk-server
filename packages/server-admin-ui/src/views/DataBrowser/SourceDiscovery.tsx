...
const SourceDiscovery: React.FC = () => {
  ...
  const [ydwgConnectionIds, setYdwgConnectionIds] = useState<Set<string>>(
    () => new Set()
  )
    ...
    fetch(`${window.serverRoutesPrefix}/providers`, {
      ...
      .then(
        (
          ...
          const ydwgIds = new Set<string>()
          for (const p of providers) {
            if (p.type === 'SignalK') {
              wsIds.add(p.id)
            }
            // Yacht Devices YDEN/YDWG gateways are known to drop discovery
            // requests (PGN 60928 / 126996 / 126998) intermittently — not
            // a UDP vs TCP issue, the gateway itself swallows them. Flag
            // the provider so the UI can explain why some identity
            // fields may stay empty and recommend retrying discovery.
            const subType = p.options?.type
            if (typeof subType === 'string' && /^ydwg02/.test(subType)) {
              ydwgIds.add(p.id)
            }
          }
          setWsConnectionIds(wsIds)
          setYdwgConnectionIds(ydwgIds)
        }
      )
      .catch((err) => {
  ...
  useEffect(() => {
    // Always refetch on mount: a connection added on the Connections
    // page won't push a SOURCES event, so without this the Discovery
    // page would keep showing the snapshot from when the store was
    // first populated.
    let cancelled = false
  ...
  const conflicts = useMemo(
    () => detectInstanceConflicts(devices, pgnDataInstances, pgnSourceKeys),
    [devices, pgnDataInstances, pgnSourceKeys]
  )
  ...
  const saveIgnored = useCallback(
    ...
    [ignoredConflicts, setIgnoredConflicts]
  )
  ...
  const handleIgnoreConflict = useCallback(
    (c: InstanceConflict) => {
      ...
      // If the filter was narrowed to the pair we just accepted, drop
      // the filter so the user is not left staring at the filtered
      // pair with no obvious way back to the full device list.
      setConflictFilter((prev) => {
        if (!prev) return prev
        if (prev.has(c.deviceA.sourceRef) && prev.has(c.deviceB.sourceRef)) {
          return null
        }
        return prev
      })
  ...
  // Cap the total wait — a 253-device bus would otherwise block the spinner
  // for ~4 minutes when most of that time is spent waiting on devices that
  // never respond.
  ...
  const handleDiscover = useCallback(() => {
    ...
    fetch(`${window.serverRoutesPrefix}/n2kDiscoverDevices`, {
      ...
      .then(async (res) => {
        ...
        return Math.min(computed, DISCOVERY_MAX_MS)
      })
  ...
  const handleReset = useCallback(async () => {
    ...
    setIsResetting(true)
    try {
      const res = await fetch(`${window.serverRoutesPrefix}/resetN2kDevices`, {
        method: 'POST',
        credentials: 'include'
      })
      if (res.ok) {
        await new Promise((r) => setTimeout(r, 300))
        await Promise.all([loadSources(), loadDeviceStatus()])
      }
    } finally {
  ...
  const handleRemoveDevice = useCallback(
    async (sourceRef: string) => {
      try {
        ...
        console.warn(`Failed to remove device ${sourceRef}:`, err)
      }
    },
    [loadSources, loadDeviceStatus]
  )
      ...
      {devices.length > 0 &&
        ...
        devices.some(
          (d) =>
            ...
            ydwgConnectionIds.has(d.connection)
        ) && (
          <Alert
            variant="warning"
            style={{ fontSize: '0.9rem', marginBottom: '16px' }}
          >
            <FontAwesomeIcon icon={faInfoCircle} /> Yacht Devices YDEN / YDWG
            gateways sometimes drop discovery PGN requests — not every device on
            the bus answers ISO Requests for 60928 / 126996 / 126998. If a
            device is missing its model, software version or installation
            description, click <strong>Discover Devices</strong> again; the
            server also retries automatically a few minutes after startup.
          </Alert>
        )}
      {devices.length > 0 && (
        <Card>
          ...
          >
            ...
            {sourceStatusLoaded && devices.length > 0 && !allWsProxied && (
              <>
            ...
            {conflictFilter && (
              <Button
                ...
                title="Clear the conflict filter and show every device again"
              >
                Show all devices
              </Button>
            )}
      ...
      <td onClick={(e) => e.stopPropagation()}>
        ...
        {isVictronDevice(device) ? (
          ...
          >
            ...
            />
            Victron devices use device instance for internal synchronization.
            Changing it may break charger coordination or ESS operation.{' '}
            <a
              href="https://www.victronenergy.com/media/pg/Cerbo_GX/en/marine-mfd-integration-by-nmea-2000.html#UUID-7b1e8a09-2b73-b046-1124-c4708e1a7563"
              target="_blank"
          ...
          >
            <FontAwesomeIcon
              ...
              style={{ marginRight: '3px' }}
            />
            Some manufacturers use device instance for internal synchronization.
            Check your device documentation before changing.
          </div>
        )}
      </td>
    )
...
const InlineTextField: React.FC<{
  ...
  const editValue =
    !isSaving && editState.syncedFrom !== currentValue
      ? currentValue
      : editState.value
