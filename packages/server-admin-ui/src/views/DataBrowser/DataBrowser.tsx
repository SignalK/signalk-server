  ...
  const filteredPathKeys: string[] = useMemo(() => {
    ...
    if (sourceFilter) {
      ...
      for (const compositeKey of filtered) {
        ...
        // Priority groups + overrides only apply to vessels.self; the
        // engine never picks a winner for AIS targets or other vessels.
        // Skip dedup for non-self rows in ALL mode so AIS sources fan
        // out as-is — their "priority filter" is identity (every source
        // intentional), not a winner choice.
        if (context === 'all' && ctxPrefix !== 'self') {
          deduped.push(compositeKey)
          continue
        }
