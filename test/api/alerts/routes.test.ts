...
describe('AlertsApi routes', () => {
  ...
  describe('list', () => {
    ...
    it('rejects a repeated group filter rather than ignoring it', async () => {
      await raise({ path: 'one', priority: 'alarm', group: 'engine' })

      expect(
        (await call(`GET ${API}`, { query: { group: ['engine', 'tanks'] } }))
          .status
      ).to.equal(400)

      // Without this, the assertion above also passes for a route that
      // rejects every group filter.
      const single = await call(`GET ${API}`, { query: { group: 'engine' } })
      expect(single.status).to.equal(200)
      expect((single.body as Alert[]).map((a) => a.path)).to.deep.equal(['one'])
    })
