...
describe('alerts deltas', () => {
  ...
  describe('ingress', () => {
    ...
    it('keeps meta on an update whose values are all alert paths', async () => {
        ...
        updates: [
          {
            ...
            values: [
              {
                path: 'alerts.tank.low' as Path,
                value: { priority: 'caution', message: 'Low' } as never
              }
            ],
            meta: [
              {
      ...
      const update = passed[0].updates[0] as {
        values?: unknown
        meta?: unknown[]
      }
      expect(update.values).to.be.undefined
      expect(update.meta).to.have.lengthOf(1)
    })
