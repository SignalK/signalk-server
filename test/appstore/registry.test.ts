...
describe('appstore/registry badgesToIndicators', () => {
  ...
  it('keeps critical severity when paired with less-severe audit badges', () => {
    // The registry can publish multiple audit-* badges at once. Make
    // sure a plugin carrying both audit-moderate and audit-critical
    // doesn't get its critical finding downgraded to warn by the
    // less-severe branch firing first.
    expect(
      badgesToIndicators(['audit-moderate', 'audit-critical'], 0).checks.find(
        (c) => c.id === 'audit'
      )?.status
    ).to.equal('fail')
    expect(
      badgesToIndicators(
        ['audit-high', 'audit-moderate', 'audit-critical'],
        0
      ).checks.find((c) => c.id === 'audit')?.status
    ).to.equal('fail')
  })
