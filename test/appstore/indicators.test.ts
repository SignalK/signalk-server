...
describe('appstore/indicators', () => {
  ...
  it('warns on a very short README rather than reporting it missing', () => {
    const r = computeIndicators({ readme: 'x'.repeat(50) })
    const c = r.checks.find((x) => x.id === 'documentation')
    expect(c?.status).to.equal('warn')
    expect(c?.subtitle).to.match(/very short/i)
  })
  ...
  it('skips GitHub-dependent checks cleanly when no repo', () => {
    ...
    const c = r.checks.find((x) => x.id === 'open-issues')
  ...
  it('returns raw metrics unchanged', () => {
    ...
    expect(r.rawMetrics.openIssues).to.equal(3)
    expect(r.rawMetrics.contributors).to.equal(5)
  })
})
