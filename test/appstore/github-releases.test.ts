...
describe('appstore/github-releases', () => {
  describe('parseGithubSlug', () => {
    ...
    it('rejects URLs that merely contain github.com as a path suffix', () => {
      // Without the anchored prefix the regex used to accept URLs
      // ending in github.com/<owner>/<repo>, which would silently send
      // release lookups to the wrong host.
      expect(parseGithubSlug('https://notgithub.com/owner/repo')).to.equal(
        undefined
      )
      expect(
        parseGithubSlug('https://example.com/redirect?to=github.com/o/r')
      ).to.equal(undefined)
    })
