...
describe('Subscriptions', _ => {
  ...
  async function testSelfData(url) {
    ...
    //3rd message is null course delta from courseApi
    const echoedDelta = await wsPromiser.nthMessage(4)
    assert(JSON.parse(echoedDelta).updates[0].source.pgn === 128275)

    try {
      await wsPromiser.nthMessage(5)
  ...
  it('?subscribe=all subscription serves all data', async function () {
    ...
    const echoedSelfDelta = await wsPromiser.nthMessage(4)
    assert(JSON.parse(echoedSelfDelta).updates[0].source.pgn === 128275)

    const echoedOtherDelta = await wsPromiser.nthMessage(5)
    assert(
