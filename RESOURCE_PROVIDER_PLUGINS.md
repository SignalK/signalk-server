...
`listResources()` returns a Promise containing a JSON object listing resources by id.
...
`getResource()` returns a Promise containing a JSON object with the resource data.
...
_Returns a Promise containing the resource data:_
```typescript
Promise<{
...
```
_Returns a Promise containing the value of `geometry` attribute of the waypoint._
```typescript
Promise<{
  "type": "Point",
...
`setResource() ` returns Promise<true> on success and Promise<null> on failure.
