...
export class HistoryApiHttpRegistry {
  ...
  private recordDefaultIfUnconfigured(pluginId: string): void {
    ...
    this.saveConfiguredProvider(candidate, (err?: Error) => {
      if (!err) {
        // Clear only if no other provider claimed the slot while this
        // write was in flight, so a replacement's pending claim survives.
        if (this.pendingDefaultProviderId === candidate) {
  ...
  unregisterHistoryApiProvider(pluginId: string): void {
    ...
    if (pluginId === this.pendingDefaultProviderId) {
      this.pendingDefaultProviderId = undefined
      // The pending provider is gone, but another one may already be
      // serving as the fallback default.  Record it so a restart does
      // not reassign the slot to whatever plugin loads first.
      const remainingId = this.historyProviders.keys().next().value
      if (remainingId !== undefined) {
        this.recordDefaultIfUnconfigured(remainingId)
      }
    }
    if (pluginId === this.configuredProviderId) {
