...
import { hasMeta, hasValues, SKVersion } from '@signalk/server-api'
  ...
  private onDelta(delta: Delta, next: (delta: Delta) => void): void {
    ...
    for (const update of delta.updates ?? []) {
      ...
      if (kept.length > 0) {
        ...
        // An update may carry both values and meta. Taking every value must
        // not take the meta entries with it.
        const { values: _values, ...withoutValues } = update
        updates.push(withoutValues)
      }
    }
