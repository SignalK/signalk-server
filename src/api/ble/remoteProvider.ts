...
class RemoteGATTSession {
  ...
  async subscribeGATT(
    ...
    if (descriptor.init) {
      cmd.init = descriptor.init.map((i) => ({
        uuid: i.uuid,
        data: i.data,
        ...(i.withResponse !== undefined && { with_response: i.withResponse })
      }))
    }
    if (descriptor.periodicWrite) {
      cmd.periodic_write = descriptor.periodicWrite.map((pw) => ({
        ...
        ...(pw.withResponse !== undefined && { with_response: pw.withResponse })
      }))
    }
    ...
    const handle: GATTSubscriptionHandle & { _fireDisconnect: () => void } = {
      ...
      write: async (charUuid: string, data: Buffer, withResponse?: boolean) => {
        if (this.ws.readyState !== WebSocket.OPEN) {
          throw new Error(
            `Gateway ${this.gatewayId} WebSocket is not open (session=${sessionId})`
          )
        }
        this.ws.send(
          JSON.stringify({
...
export class RemoteGatewayProvider {
  ...
  private _handleAdvertisementPost(req: Request, res: Response) {
    ...
    if (
      ...
      res.status(400).json({
        error: first
          ? `Invalid gateway POST body at ${first.path}: ${first.message}`
          : 'Invalid gateway POST body'
      })
      return
    }
