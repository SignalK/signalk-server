...
const debug = createDebug('signalk-server:api:gpsOffsetCorrector')
...
export interface GpsCorrectorApplication
  extends
    IRouter,
    ConfigApp,
    WithConfig,
...
// Server-side lever-arm correction for GPS antenna offsets.
//
// Rewrites navigation.position deltas whose $source matches a configured
// gpsSensors row, so downstream consumers (full data model, streambundle,
...
// gpsOffsetCorrection so history plugins can reconstruct the antenna's
...
export class GpsOffsetCorrector {
  ...
  private rebuildLookup(): void {
    ...
    const sensors = this.app.config.settings.gpsSensors ?? []
    for (const s of sensors) {
      // Skip rows that cannot drive a correctness-preserving correction:
      //   - empty sourceRef would otherwise hijack every position delta;
  ...
  private handleUpdate(update: Update): void {
    ...
    for (const pv of update.values) {
      ...
      entry = this.lookup.get(sourceRef)
      if (entry) {
    ...
    const correction: GpsOffsetCorrection = {
    ...
    const metaEntry: Meta = {
      ...
      value: { gpsOffsetCorrection: correction }
    }
