...
// rather than inside the appstore() closure because the function-defs
// after the closure's `return { ... }` are reachable (hoisted) but
...
module.exports = function (app) {
  ...
  return {
    start: function () {
      ...
      app.post(`${SERVERROUTESPREFIX}/appstore/refresh`, (req, res) => {
        ...
        // A manual refresh should bypass the warmup cooldown, otherwise
        // the next /appstore/available will skip the installed-detail
      ...
      buildPluginDetail(
        {
          ...
          categories: getCategories(pkgForEnrichment),
  ...
  // Each loadPluginDetail() rebuilds the full plugin/webapp module list
  // before fetching readme/changelog/metrics, so calling it once per
  // installed plugin per /appstore/available hit is expensive: with N
  // installed plugins and overlapping list requests, this fans out to
  // O(N) discovery passes plus their downstream I/O. Guard with an
  // in-flight flag so concurrent triggers collapse, and a per-pass
  // cooldown so a list endpoint that fires every few seconds doesn't
  // re-issue a fresh refresh on each call. State lives at module scope
  // (see top of file) because this closure already returned the public
  // API by the time scheduleInstalledDetailRefresh is reachable.
  function scheduleInstalledDetailRefresh() {
    if (installedDetailRefreshInFlight) return
    if (
      Date.now() - installedDetailRefreshLastRunAt <
      INSTALLED_DETAIL_REFRESH_COOLDOWN_MS
    ) {
      return
    }
    const installedNames = getInstalledPackageNames()
    if (installedNames.length === 0) return
    installedDetailRefreshInFlight = true
    setImmediate(async () => {
      try {
        await Promise.all(
        ...
        installedDetailRefreshInFlight = false
        installedDetailRefreshLastRunAt = Date.now()
      }
    })
  }
  ...
  function getModulesInfo(modules, existing, result, distTagsMap, regLookup) {
    modules.forEach((plugin) => {
      ...
      const pluginInfo = {
        ...
        recent: isRecent(plugin.package.date),
  ...
  function updateSKModule(module, version, isRemove, pluginId, deleteData) {
    // Drop the metadata cache entry up front so any read that comes in
    // while the install is in flight (status pings, refresh requests)
