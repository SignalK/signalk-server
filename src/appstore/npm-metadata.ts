...
async function fetchPackageMetadata(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // Only treat true not-found / forbidden / gone as definitive. 429
      // (rate-limited) is transient and must keep the stale-on-disk
      // fallback path open so a throttled npm response doesn't blow away
      // hours of cached metadata.
