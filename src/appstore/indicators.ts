...
// Per-check weight by check id (kebab-case to match the ids the
// scoring functions emit). Single source of truth for the composite.
...
function scoreOpenIssues(inputs: IndicatorInputs): IndicatorCheck {
  if (!inputs.hasRepository || inputs.openIssues === undefined) {
    return neutralCheck('open-issues', 'Open issues')
  }
...
function scoreDocumentation(inputs: IndicatorInputs): IndicatorCheck {
  ...
  // The 'len === 0 && !hasDesc' case returned neutralCheck above; by here
  // !hasDesc is true and len > 0, so the README is present but below the
  // warn threshold.
  return {
    id: 'documentation',
    status: 'warn',
    title: 'Documentation coverage',
    subtitle: 'README is present but very short'
  }
}
