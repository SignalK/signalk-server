...
export function badgesToIndicators(
  ...
  // The registry can publish multiple audit-* badges per plugin (one
  // for each severity level present), so check the most severe first.
  // Otherwise a plugin carrying both `audit-moderate` and
  // `audit-critical` would have rendered as 'warn' instead of 'fail'.
