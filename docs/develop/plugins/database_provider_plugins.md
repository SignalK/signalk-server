...
The server ships two built-in SQLite providers (`_builtin` via better-sqlite3, `_builtin_nodesqlite` via node:sqlite). A database provider plugin can register an alternative backend — for example PostgreSQL — to replace or supplement the built-in providers.
...
A plugin registers itself as a database provider by calling the server's {@link @signalk/server-api!DatabaseApiRegistry.registerDatabaseProvider | `registerDatabaseProvider`} function during startup.
...
When a provider is registered, it becomes the default provider. When unregistered (e.g. plugin stopped), the server falls back to the built-in SQLite provider.
