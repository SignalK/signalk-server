...
- **duration**: Duration of the time range in seconds (integer) or as an ISO8601 Duration string (e.g. `PT15M`). Can be specified with either 'from' or 'to'. If they are both omitted is relative to 'now'.
...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| `paths`      | Comma separated list of Signal K paths whose data should be retrieved. Optional aggregation methods for each path as postfix separated by a colon. Aggregation methods: 'average' \| 'min' \| 'max' \| 'first' \| 'last' \| 'mid' \| 'middle_index' \| 'sma' \| 'ema'. The 'sma' and 'ema' methods accept an optional numeric parameter as a second postfix (`path:method:parameter`): for `sma` it is the window size in samples, for `ema` it is the alpha value (0-1). When the parameter is omitted the provider applies its own default. The default aggregate is `average`, except for the object-valued path `navigation.position`, where the default is `first`. An explicit aggregate is always passed to the provider unchanged, including choices like `average` on `navigation.position` that may not be meaningful for an object-valued path; how the provider responds in that case is provider-specific. Example: `navigation.speedOverGround:sma:5,navigation.speedThroughWater:max` | Yes      |
| `context`    | Signal K context that the data is about, defaults to 'vessels.self'. Example: `vessels.urn:mrn:imo:mmsi:123456789`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | No       |
| `resolution` | Length of data sample time window in seconds or as a time expression ('1s', '1m', '1h', '1d'). If resolution is not specified the server should provide data in a reasonable time resolution, depending on the time range in the request.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | No       |
...
A path whose units are `rad` in the path metadata — a heading, a course, a wind angle — is averaged as a vector: `average`, `sma` and `ema` take the angle of the mean sine and cosine, so 359° and 1° average to 0°, not 180°. The result keeps the path's own convention, `[0, 2π)` or `(−π, π]`; samples that cancel, 0° and 180° in equal measure, have no mean direction and the bucket is `null`. `min`, `max` and `mid` are arithmetic.
