...
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| `paths`      | Comma separated list of Signal K paths whose data should be retrieved. Optional aggregation methods for each path as postfix separated by a colon. Aggregation methods: 'average' \| 'min' \| 'max' \| 'first' \| 'last' \| 'mid' \| 'middle_index' \| 'sma' \| 'ema'. The 'sma' and 'ema' methods accept an optional numeric parameter as a second postfix (`path:method:parameter`): for `sma` it is the window size in samples, for `ema` it is the alpha value (0-1). When the parameter is omitted the provider applies its own default. Example: `navigation.speedOverGround:sma:5,navigation.speedThroughWater:max` | Yes      |
